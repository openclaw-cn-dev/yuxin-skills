...
v3 (2026-08-14 20:35): 增加 baostock 数据源 fallback，腾讯 K 线被 WAF 封后启用
"""
...
def fetch_kline(code, days=130):
    """优先腾讯 K 线 (2026-08-14 被 WAF 封后), 回退 baostock。
    code 格式: sz000768 / sh600519 / bj920808"""
    # 1) 尝试腾讯
    try:
        ...
        # 腾讯 WAF 封禁返回 HTML, 不是 JSON
        if body.startswith('<') or 'waf.tencent' in body or '501' in body:
            raise Exception('tencent WAF blocked')
    ...
    if not HAS_BS:
        return []
    try:
        # 沙箱内 baostock 全局只能 login 一次
        if not hasattr(fetch_kline, '_bs_logged'):
            bs.login()
            fetch_kline._bs_logged = True
        bs_code = ('sh.' if code.startswith('sh') else 'sz.') + code[2:]
        rs = bs.query_history_k_data_plus(
            bs_code,
            'date,open,high,low,close,volume',
            start_date='2025-08-01', end_date='2026-12-31',
            adjustflag='2', frequency='d')
        rows = []
        while rs.next():
            row = rs.get_row_data()
            # baostock 字段: date,open,high,low,close,volume (6 列)
            # 转为腾讯格式: [date,open,close,high,low,volume]
            if len(row) == 6:
                rows.append([row[0], row[1], row[4], row[2], row[3], row[5]])
        return rows[-days:] if rows else []
    except Exception as e:
        log(f'[WARN] baostock {code} fail: {e}')
        return []
