...
"""
宽博士因子扩展版全 A 扫描 — 30 因子 + 200 池 + IC 验证 (东方财富版)
====================================================
...
v4 (2026-08-14 20:55): 东方财富 curl 数据源, 绕过腾讯 WAF 封禁 + baostock 反爬
"""
...
# 数据源优先级: 腾讯K线 -> 东方财富K线(curl) -> baostock
# 2026-08-14 20:30 腾讯 WAF 封禁, 改用东方财富
...
def _em_secid(code):
    """sz/sh 前缀转东方财富 secid: 1.600xxx / 0.000xxx"""
    prefix = code[:2]
    num = code[2:]
    market = '1' if prefix == 'sh' else '0'
    return f'{market}.{num}'

def _em_fetch_via_curl(code, days=130):
    """东方财富 K 线 - curl 落盘, Python 解析 (绕过反爬)"""
    secid = _em_secid(code)
    url = (f'https://push2his.eastmoney.com/api/qt/stock/kline/get?'
           f'secid={secid}&fields1=f1,f2&fields2=f51,f52,f53,f54,f55,f56'
           f'&klt=101&fqt=1&beg=0&end=20500000')
    try:
        ...
        # 我们需要的腾讯格式: [date,open,close,high,low,volume]
        rows = []
        ...
        log(f'[WARN] em curl {code}: {e}')
        return []


def fetch_kline(code, days=130):
    """优先腾讯 K 线 (2026-08-14 被 WAF 封后), 回退东方财富 curl。
    code 格式: sz000768 / sh600519"""
    # 1) 尝试腾讯 (5s 超时, WAF 封后很快失败)
    try:
    ...
    # 2) 回退东方财富 curl
    return _em_fetch_via_curl(code, days)
