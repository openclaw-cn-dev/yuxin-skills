...
- **macos-voice-interaction** (devops/) — macOS 语音交互 4 阶段实现 + Porcupine 唤醒词（2026-07-02 实测）。Unique content：🔴 核心结论「通用 ASR（vosk 小/大、whisper）对"玉芬"这类专有名词天然识别失败（识别成"一分/与分"），**只有声纹匹配（Porcupine）能稳定识别**，下次直接推 Picovoice，不要在通用 ASR 上浪费时间」；Hammerspoon 菜单栏 + LaunchAgent 双进程保活（LaunchAgent 后台进程可能拿不到麦克风 TCC 权限，Hammerspoon 跑用户 session 兜底，但双进程不能同时跑会抢麦克风）；Picovoice Console 训练中文唤醒词 5 步 SOP（language 必须选 zh）；三层崩溃告警（L1 callback try/except + L2 /tmp crash 文件 + L3 cron watcher + L4 卡死 watchdog）。支持文件（templates/init.lua、references/picovoice-console-setup.md、scripts/install_porcupine_license.sh 等）已随包归档到 .archive/。
- **feishu-voice-assistant** — 飞书语音助手 YVA（"语音消息 → Agent → 语音回复"四层链路，ASR→Agent→TTS→IM）。Unique content：DashScope/CosyVoice/ElevenLabs 选型表、飞书 webhook 异步编排 + 幂等、Phase 1-3 路径（飞书 MVP → Mac 客户端 → 养殖场硬件）、v4 战略卡位。正文 `references/feishu-voice-assistant.md`，案例 `references/***SECRET***.md`，模板 `templates/feishu-voice-webhook.py`、`templates/yva-mvp-server.py`。
- **browser-tts-lecture** — 浏览器 SpeechSynthesis API 生成中文讲解语音（零成本、分段朗读 + 视觉高亮同步 + lecture/quick 双模式）。Unique content：`tts` 核心引擎 JS 代码、视觉高亮 CSS、Chrome 需用户手势等 5 项 pitfall。正文 `references/browser-speechsynthesis-tts.md`。
