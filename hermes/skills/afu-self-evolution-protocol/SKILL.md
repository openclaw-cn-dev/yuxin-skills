...
description: 阿福（渔芯客服）自我进化工作流协议 — 每次 cron 心跳跑"无任务→自进化"时必须遵循的 5 步骤流程、6 文件级联升级模式、战术 N+1 缝隙识别节奏、元数据三方一致纪律、patch 工作流陷阱、自检脚本 verify_evolution.py v2.0。触发条件：阿福 cron 跑出"无任务"分支、heartbeat_check.py 静默返回、agent 准备进入 evolution mode、agent 需要 patch 已有 skill 的 SKILL.md frontmatter、agent 沉淀新谈判战术或新 RAS 弹药。
version: 1.3.0
author: 渔芯科技 / 阿福
tags: [阿福, 自进化, 心跳, 元数据, patch, 陷阱, SOP, 战术缝隙, 6文件级联]
changelog:
...
> **本 skill 的存在理由**：阿福每 4 小时跑一次 cron 自我进化轮，从 8/25 当日已经跑了 5+ 轮（04:00/08:00/12:00/16:00/20:00），8/26 当日已跑 2 轮（00:00 / 04:00），期间反复出现 3 类本可避免的失误。本 skill 把这些失误沉淀为 SOP，让下一次进化轮直接按表执行。
...
**8/26 04:00 实测**：连续 2 期（8/26 00:00 + 8/26 04:00）三方真闭环——这是阿福 cron 进化史上首次。
...
### 3.5 战术 N+1 缝隙识别节奏 🆕（8/26 04:00 实测 3 次验证）🆕（8/26 08:00 第 4 次验证）

**模式**：每次新战术（#N）落地后，**必扫下一个缝隙（#N+1）**——节奏已 **4 次验证**（8/26 08:00 新增战术十三 Hard Date Anchoring 验证）：
...
**4h 节奏稳定性证据**：连续 4 次都是 4 小时间隔——说明 SOP 已稳定到"自动发现缝隙"的程度，不是偶然命中。下次心跳继续验证是否仍是 4h 节奏；如果出现 8h 间隔，说明节奏被其他任务挤压，需要在 §1 步骤 3 加优先级强化。
...
- 战术十二（Pre-mortem）→ Pre-mortem 后仍拖延 → **战术十三（强制承诺日·Hard Date Anchoring）** ← 下期沉淀目标

### 3.6 「战术使用顺序」类 pitfall 🆕（8/26 04:00 实测新增 Pitfall 9）

**8/26 04:00 新发现的 pitfall 类别**——不只是工程类 pitfall，还有**战术使用顺序类 pitfall**：
...
**沉淀规则**：每次新战术章节末尾，必须问"这个战术如果在 [前序战术] 之前使用会发生什么"——如果会引发副作用，立即加 1 条 Pitfall 到 `afu-patch-pitfalls.md`。

**Pitfall 9 实测位置**：`negotiation-voss-techniques/references/afu-patch-pitfalls.md`（已落地，详见 §3.4 的脚本扫描）
...
- 下次更新触发：① 当出现新的 patch 失误模式时 → §3 追加新陷阱 ② 当战术 #N+1 落地时 → §3.5 追加新节奏 ③ 当三方一致失守时 → §2.5 追加新失败模式
- 与已有 skill 的关系：
