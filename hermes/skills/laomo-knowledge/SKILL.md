...
**v1.75.0** (2026-09-06 04:13 CST) — R243 self-evolution round 新增 Pitfall #48 + PBT 协议升级（详见 `references/***SECRET***.md`）。PBT Property 5 redesign 4 原则 + P1 白名单加 404 + dual-track R229→R243 跳号 13h8m 实战。版本 bump v1.74.0 → v1.75.0。

**v1.77.0** R251 self-evolution round: Pitfall #49 (LLM GW /health body 缺 status:ok 字段, PBT P5 5/10 FAIL) + Pitfall #4 扩展 (Crossref SOURCE OF TRUTH) + ref `r251-*.md`. v1.76.0→v1.77.0。 — R245 self-evolution round 新增 1 个 reference + 2 个实战沉淀。**(a) known_dois.txt 累积认知偏差 Pitfall #39 变体 #2**: R244 entry 写「357 DOI」/ R245 实测 395 行 (差额 +38 行其他 R<n> 累积)，证明 R<n> entry 中数字也会随时间陈旧, 不只路径存在性偏差。R245 防御：涉及 known_dois.txt 数字时必先 wc -l 实测, 写「R<N> 前实测 N 行 / R<N> 后 N+X 行」带时间戳。**(b) PBT 6 properties 跨服务可比性**: R245 首次跑非 LLM GW 服务 (老莫 :8006)，method × status 分布 PUT 405 vs LLM GW PUT 200 = 老莫 method 白名单严于 LLM GW (Pitfall #41 反例对照)，印证 PBT 横向比较 method policy 差异价值。**(c) R199 → R245 老莫 uvicorn 服务恢复反向跟踪**: status=-1 → 200 反向轨迹补 Pitfall #35/44 双向证据 (正方向: 形式 PASS ≠ 实质健康 / 反方向: 实质 FAIL 不必全 PASS)。新增 reference `references/***SECRET***.md` (含 6 章节：累积偏差变体 / 跨服务 PBT 对比 / 反向跟踪表 / cron delivery 格式约定 / R246+ SOP / 沉淀物清单)。**版本 bump v1.75.0 → v1.76.0**。

**v1.74.0** (2026-09-04 19:02 CST)
