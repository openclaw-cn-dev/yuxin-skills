...
**Practical competitor-inspired workflow:** When you want to borrow features from established products (Anki, Quizlet, Duolingo, etc.), follow the structured process in [references/competitor-inspired-optimization.md](references/competitor-inspired-optimization.md) — benchmark selection → value/cost scoring → "do less" filtering → clean versioned shipping. This prevents feature-for-feature bloat while delivering maximum user value.
...
- [references/competitor-inspired-optimization.md](references/competitor-inspired-optimization.md) — Practical workflow for borrowing the best features from competitors without bloating your product: benchmark selection → value/cost scoring → "do less" filtering → clean versioned shipping. Includes a complete example from the Anki/Quizlet/Duolingo/Memrise flashcard iteration.
