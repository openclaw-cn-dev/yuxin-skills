...
### 2026-09-12 同城门店引流实测补充——README 总纲流派 + 冷启动首跑

**目录骨架两种流派**（按项目形态选）：
- INDEX.md 流派（OPC 出海）：单主线调研，INDEX 做定义/框架/进度
- **README.md 总纲流派**（AI法典/创意之神/同城门店引流）：多方向轮换调研。README 写一句话定位 + 核心假设 + 调研重点 N 个方向（编号轮换）+ 目录说明 + 底部"调研进度登记表"（日期|方向|报告文件|关键结论一句）；分报告进 `01-研究报告/NN-方向名.md`。多方向轮换项目默认用此流派

**华哥带豆包初稿立项时**（常见模式：豆包出初稿→让玉芬"综合整理"，须交叉核查不盲信）：
1. 原文存档 `02-豆包初稿.md`，头部标注"待交叉核查，仅作调研起点假设"
2. 核心假设压缩并入 README（标注"豆包初稿，待交叉核查"）
3. **每轮报告必含"对豆包初稿的验证/修正"表**：逐条标 ✅证实 / ⚠️需修正 / ❌推翻 / ⏸延后——这是给华哥的核心增量，豆包对不对本身就有汇报价值

**冷启动首跑 SOP**（建完 cron 别干等首个排期点）：
1. `cronjob action=run` 立即触发首轮（实测 2-4 分钟出报告；手动触发的完成信号是**报告文件落盘**，cron 的 output 目录/last_status 都不会即时反映本轮）
2. 轮询等落盘：`for i in 1..6; do sleep 60; [ -n "$(ls -A <报告目录>)" ] && break; done`
   - **坑**：前台 sleep 循环受 600s 上限约束，轮次 >6 分钟或要等多轮时改用 `terminal(background=true, notify_on_complete=true)` 起监听循环 + `process(action='wait')` 收结果，不占前台额度
3. **亲自读报告验收**再向华哥报"已落地"——cron last_status=ok 只代表跑完，不代表产出质量；抽查 TL;DR 与来源链接
4. 首轮结论直接进对华哥的立项汇报（比"已建好 cron"有料得多）

**调研频率档位与额度联动**（2026-09-12 实操固化）：
- 华哥说"套餐额度充足，提高采集调研频率" → `cronjob list` 找调研组（zhenglishi 线 7 个：RAS仿真/360行/四技术/梦幻西游/Kali/AI法典/创意之神）→ 整组 6h→3h（`every 180m` 或 `0 */3 * * *`）
- 额度再紧张 → 一键降回 `every 360m`（8/22 省 token 时期档位）。心跳/watchdog/监控类永远 no_agent 不动
- 改完 Mnemosyne 登记改动清单+回退路径；与既有 2h/1h 高频任务不叠加提频
