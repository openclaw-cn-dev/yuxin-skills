...
配套 cron:
  ...
  - no_agent: True
  - 静默策略:有变更才输出报告,无变更 → 空 stdout → 飞书不发
  - workdir: /Users/hua

排除路径(RKR 资源/EDAI 测试/个人脚本):
  - 302-数据与素材库/、antigravity-awesome-skills/、awesome-claude-code/、
    awesome-agent-skills/、everything-claude-code/、cad_samples/、
    material_library/、mech_drawing/、mech_drawing_reverse/、
    freecad-automation/(老莫私有)、career-ops/、pipeline/、research/、
    skills/、video_understanding/、voiceprint_samples/、web_search/、
    EDAI自动审批测试/

也保存一份到 ~/.hermes/state/workspace_audit_latest.md 供后续查询。
"""
import sys
